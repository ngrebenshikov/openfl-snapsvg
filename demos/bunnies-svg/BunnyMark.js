...
ApplicationMain.main = function() {
	...
	ApplicationMain.loaders.set("assets/grass.png",loader);
	...
	ApplicationMain.loaders.set("assets/pirate.png",loader1);
	ApplicationMain.total++;
	var urlLoader1 = new openfl.net.URLLoader();
	urlLoader1.set_dataFormat(openfl.net.URLLoaderDataFormat.BINARY);
	ApplicationMain.urlLoaders.set("assets/rabbit.svg",urlLoader1);
	ApplicationMain.total++;
	var loader2 = new openfl.display.Loader();
	ApplicationMain.loaders.set("assets/wabbit_alpha.png",loader2);
	ApplicationMain.total++;
	...
	if(ApplicationMain.total == 0) ApplicationMain.begin(); else {
		...
		while( $it0.hasNext() ) {
			...
			loader3.load(new openfl.net.URLRequest(path));
		}
		...
		while( $it1.hasNext() ) {
			...
			urlLoader2.load(new openfl.net.URLRequest(path1));
		}
	}
};
...
ApplicationMain.preloader_onComplete = function(event) {
	...
	if(Reflect.field(BunnyMark,"main") == null) {
	...
	} else Reflect.callMethod(BunnyMark,Reflect.field(BunnyMark,"main"),[]);
};
var openfl = {};
openfl.events = {};
...
var BunnyMark = function() {
	...
	Env.setup();
	openfl.Lib.get_current().set_scaleX(openfl.Lib.get_current().set_scaleY(Env.screenDensity));
	this.background = new Background();
	this.background.texture = openfl.Assets.getBitmapData("assets/grass.png");
	this.background.cols = 8;
	this.background.rows = 12;
	this.background.set_x(-50);
	this.background.set_y(-50);
	this.background.setSize(Env.width + 100,Env.height + 100);
	this.addChild(this.background);
	this.addChild(new tests.SvgTest());
	this.fps = new openfl.display.FPS();
	this.addChild(this.fps);
	this.fps.addEventListener(openfl.events.MouseEvent.CLICK,$bind(this,this.fps_onClick));
	this.get_stage().addEventListener(openfl.events.Event.RESIZE,$bind(this,this.stage_onResize));
};
$hxClasses["BunnyMark"] = BunnyMark;
BunnyMark.__name__ = ["BunnyMark"];
BunnyMark.__super__ = openfl.display.Sprite;
BunnyMark.prototype = $extend(openfl.display.Sprite.prototype,{
	fps_onClick: function(event) {
		this.get_stage().set_frameRate(90 - this.get_stage().get_frameRate());
	}
	,stage_onResize: function(event) {
		if(Env.width > Env.height) {
			this.background.cols = 12;
			this.background.rows = 8;
		} else {
			this.background.cols = 8;
			this.background.rows = 12;
		}
...
DocumentClass.__super__ = BunnyMark;
DocumentClass.prototype = $extend(BunnyMark.prototype,{
	get_stage: function() {
...
Background.__super__ = openfl.display.Sprite;
Background.prototype = $extend(openfl.display.Sprite.prototype,{
	build: function() {
	}
	,paint: function() {
		this.get_graphics().clear();
		this.get_graphics().beginBitmapFill(this.texture);
		this.get_graphics().drawRect(0,0,this.get_stage().get_stageWidth() * 1.2,this.get_stage().get_stageHeight() * 1.2);
	...
	,setSize: function(width,height) {
		...
		this._height = height;
		this.build();
		if(this.parent != null) this.paint();
	}
...
var DefaultAssetLibrary = function() {
	...
	DefaultAssetLibrary.path.set("assets/grass.png","assets/grass.png");
	var value = Reflect.field(openfl.AssetType,"image".toUpperCase());
	DefaultAssetLibrary.type.set("assets/grass.png",value);
	DefaultAssetLibrary.path.set("assets/openfl.svg","assets/openfl.svg");
	var value1 = Reflect.field(openfl.AssetType,"text".toUpperCase());
	DefaultAssetLibrary.type.set("assets/openfl.svg",value1);
	DefaultAssetLibrary.path.set("assets/pirate.png","assets/pirate.png");
	var value2 = Reflect.field(openfl.AssetType,"image".toUpperCase());
	DefaultAssetLibrary.type.set("assets/pirate.png",value2);
	DefaultAssetLibrary.path.set("assets/rabbit.svg","assets/rabbit.svg");
	var value3 = Reflect.field(openfl.AssetType,"text".toUpperCase());
	DefaultAssetLibrary.type.set("assets/rabbit.svg",value3);
	DefaultAssetLibrary.path.set("assets/wabbit_alpha.png","assets/wabbit_alpha.png");
	var value4 = Reflect.field(openfl.AssetType,"image".toUpperCase());
	DefaultAssetLibrary.type.set("assets/wabbit_alpha.png",value4);
};
...
Env.setup = function() {
	Env.screenDensity = 1;
	Env.stage_onResize(null);
	openfl.Lib.get_current().get_stage().addEventListener(openfl.events.Event.RESIZE,Env.stage_onResize);
};
Env.stage_onResize = function(event) {
	Env.width = Math.ceil(openfl.Lib.get_current().get_stage().get_stageWidth() / Env.screenDensity);
	Env.height = Math.ceil(openfl.Lib.get_current().get_stage().get_stageHeight() / Env.screenDensity);
};
...
openfl.Assets.getText = function(id) {
	...
	if(library != null) {
		if(library.exists(symbolName,openfl.AssetType.TEXT)) {
			if(library.isLocal(symbolName,openfl.AssetType.TEXT)) return library.getText(symbolName); else console.log("[openfl.Assets] String asset \"" + id + "\" exists, but only asynchronously");
		} else console.log("[openfl.Assets] There is no String asset with an ID of \"" + id + "\"");
	} else console.log("[openfl.Assets] There is no asset library named \"" + libraryName + "\"");
	return null;
};
...
openfl.display.Svg.__name__ = ["openfl","display","Svg"];
openfl.display.Svg.__super__ = openfl.display.Shape;
openfl.display.Svg.prototype = $extend(openfl.display.Shape.prototype,{
	toString: function() {
		return "[Svg name=" + this.name + " id=" + this.___id + "]";
	}
	,validateBounds: function() {
		if(this.get__boundsInvalid()) {
			openfl.display.Shape.prototype.validateBounds.call(this);
			var rect = this.snap.getBBox();
			var r = new openfl.geom.Rectangle(0,0,rect.width,rect.height);
			if(r.width != 0 || r.height != 0) {
				if(this.__boundsRect.width == 0 && this.__boundsRect.height == 0) this.__boundsRect = r.clone(); else this.__boundsRect.extendBounds(r);
			}
			if(this.scale9Grid != null) {
	...
	,__render: function(inMask,clipRect) {
		...
		if(inMask != null) {
			...
			if(null != this.get_mask() && (null == snapMask || "none" == snapMask)) {
				if(null != this.get_mask().snap) {
					if(!this.get_mask().__isOnStage()) {
						console.log("add child");
						openfl.Lib.get_current().addChild(this.get_mask());
					}
					this.snap.attr({ mask : this.get_mask().snap});
				}
			} else if(null == this.get_mask()) {
...
var tests = {};
tests.SvgTest = function() {
	openfl.display.Sprite.call(this);
	this.minX = 0;
	this.maxX = openfl.Lib.get_current().get_stage().get_stageWidth();
	this.minY = 0;
	this.maxY = openfl.Lib.get_current().get_stage().get_stageHeight();
	this.gravity = 0.5;
	this.incBunnies = 100;
	this.smooth = true;
	this.numBunnies = 180;
	this.incBunnies = 20;
	var doc = haxe.xml.Parser.parse(openfl.Assets.getBytes("assets/rabbit.svg").toString());
	var buf = new StringBuf();
	var $it0 = doc.firstElement().elements();
	while( $it0.hasNext() ) {
		var el = $it0.next();
		if(el.get_nodeName() == "metadata") continue;
		buf.add(el.toString());
	}
	this.bunnyAsset = ((function($this) {
		var $r;
		var _this = openfl.Lib.get_snap();
		var e123 = [];
		var me123 = _this;
		$r = me123.group.apply(me123, e123);
		return $r;
	}(this))).append(Snap.parse(buf.b));
	openfl.Lib.get_freeSnap().append(this.bunnyAsset);
	this.pirate = new openfl.display.Bitmap(openfl.Assets.getBitmapData("assets/pirate.png"),openfl.display.PixelSnapping.AUTO,true);
	this.pirate.set_scaleX(this.pirate.set_scaleY(Env.height / 768));
	...
	while(_g1 < _g) {
		var i = _g1++;
		bunny = new Bunny();
		bunny.position = new openfl.geom.Point();
		bunny.speedX = Math.random() * 5;
		bunny.speedY = Math.random() * 5 - 2.5;
		bunny.scale = 0.3 + Math.random();
		bunny.rotation = 15 - Math.random() * 30;
		this.bunnies.push(bunny);
		svg = new openfl.display.Svg(this.bunnyAsset["use"]());
		this.svgs.push(svg);
		this.addChildAt(svg,0);
	}
	this.createCounter();
	this.addEventListener(openfl.events.Event.ENTER_FRAME,$bind(this,this.enterFrame));
	openfl.Lib.get_current().get_stage().addEventListener(openfl.events.Event.RESIZE,$bind(this,this.stage_resize));
	this.stage_resize(null);
};
$hxClasses["tests.SvgTest"] = tests.SvgTest;
tests.SvgTest.__name__ = ["tests","SvgTest"];
tests.SvgTest.__super__ = openfl.display.Sprite;
tests.SvgTest.prototype = $extend(openfl.display.Sprite.prototype,{
	createCounter: function() {
		var format = new openfl.text.TextFormat("_sans",20,0,true);
		format.align = openfl.text.TextFormatAlign.RIGHT;
		this.tf = new openfl.text.TextField();
		this.tf.selectable = false;
		this.tf.set_defaultTextFormat(format);
		this.tf.set_width(200);
		this.tf.set_height(60);
		this.tf.set_x(this.maxX - this.tf.get_width() - 10);
		this.tf.set_y(10);
		this.addChild(this.tf);
		this.tf.addEventListener(openfl.events.MouseEvent.CLICK,$bind(this,this.counter_click));
	}
	,counter_click: function(e) {
		if(this.numBunnies >= 1500) this.incBunnies = 250;
		var more = this.numBunnies + this.incBunnies;
		var bunny;
		var svg;
		var _g = this.numBunnies;
		while(_g < more) {
		...
		this.numBunnies = more;
		this.stage_resize(null);
	}
	,stage_resize: function(e) {
		this.maxX = Env.width;
		this.maxY = Env.height;
		this.tf.set_text("Bunnies:\n" + this.numBunnies);
		this.tf.set_x(this.maxX - this.tf.get_width() - 10);
	}
	,enterFrame: function(e) {
		...
		while(_g1 < _g) {
			...
			bunny.alpha = 0.3 + 0.7 * bunny.position.y / this.maxY;
			...
			svg = this.svgs[i];
			svg.set_x(bunny.position.x);
			svg.set_y(bunny.position.y);
			svg.set_rotation(bunny.rotation);
			svg.set_alpha(bunny.alpha);
			svg.set_scaleX(svg.set_scaleY(bunny.scale));
		}
		var t = openfl.Lib.getTimer();
		this.pirate.set_x(Std["int"]((Env.width - this.pirate.get_width()) * (0.5 + 0.5 * Math.sin(t / 3000))));
		this.pirate.set_y(Std["int"](Env.height - this.pirate.get_height() + 70 - 30 * Math.sin(t / 100)));
	}
	,__class__: tests.SvgTest
});
var tools = {};
